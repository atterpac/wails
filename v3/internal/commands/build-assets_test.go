	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !tt.wantErr {
				...
				if _, err := os.Stat(filepath.Join(buildDir, "Taskfile.yml")); !os.IsNotExist(err) {
					t.Errorf("compatibility Taskfile should not be generated by default")
				}
				for _, platform := range []string{"darwin", "windows", "linux", "android", "ios"} {
					if _, err := os.Stat(filepath.Join(buildDir, platform, "Taskfile.yml")); !os.IsNotExist(err) {
						t.Errorf("%s compatibility Taskfile should not be generated by default", platform)
					}
				}
...
func TestGenerateBuildAssetsCanRetainLegacyTaskfiles(t *testing.T) {
	buildDir := filepath.Join(t.TempDir(), "build")
	err := GenerateBuildAssets(&BuildAssetsOptions{
		Dir: buildDir, Name: "Legacy", ProductName: "Legacy", ProductVersion: "1.0.0",
		ProductCompany: "Example", LegacyTaskfiles: true, Silent: true,
	})
	if err != nil {
		t.Fatalf("GenerateBuildAssets() error = %v", err)
	}
	for _, relative := range []string{"Taskfile.yml", "darwin/Taskfile.yml", "windows/Taskfile.yml", "linux/Taskfile.yml", "android/Taskfile.yml", "ios/Taskfile.yml"} {
		if _, err := os.Stat(filepath.Join(buildDir, relative)); err != nil {
			t.Errorf("expected compatibility Taskfile %s: %v", relative, err)
		}
	}
}
